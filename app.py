...
class MedicalChatbot:
    def __init__(self):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.2-1B", use_auth_token=api_token)
        self.model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-1B", use_auth_token=api_token)
        self.conversation_pipeline = pipeline("text-generation", model=self.model, tokenizer=self.tokenizer, max_length=500)
        self.conversation_history = []

    def get_answer(self, question, context):
        # Format the prompt
        prompt = f"You are a medical expert. Answer based on the following context:\n\nContext: {context}\nQuestion: {question}\nAnswer:"

        # Generate the response
        response = self.conversation_pipeline(prompt, max_length=200, num_return_sequences=1, do_sample=True)
